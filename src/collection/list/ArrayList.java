...
/**
 * 可变数组实现了List接口.实现了所有的可选列表操作, 并且允许所有元素, 包括null.
 * 除了实现List接口外, 此类还提供一些方法来操作内部用来存储列表的数组的大小(此类 约等效于 Vector, 除了此类是不同步的.)
 * size,isEmpty,get,set,iterator,listIterator 操作都以固定时间运行.
 * add 操作以分摊的固定时间 运行，也就是说，添加 n 个元素需要 O(n) 时间。
 * 其他所有操作都以线性时间运行（大体上讲）。与用于 LinkedList 实现的常数因子相比，此实现的常数因子较低。
 *
 * 对一个LinkedList做随机访问所消耗的时间与这个list的大小是成比例的。在ArrayList中进行随机访问所消耗的时间是固定的。
 *
...
public class ArrayList<E> extends AbstractList<E> implements List<E>, RandomAccess, Cloneable, java.io.Serializable
{
    ...
    /**
     * ArrayList元素的数组缓冲区存储,ArrayList的容量是该Object[]类型数组的长度.
     * 当第一个元素被添加时，任何空ArrayList中的 elementData == DEFAULTCAPACITY_EMPTY_ELEMENTDATA
     * 将会被扩充到 DEFAULT_CAPACITY（默认容量）。
     */
    transient Object[] elementData; // 没有private是为了简化嵌套类访问
    ...
    /**
     * 调整size 和当前List列表保存的实际元素大小一样  应用程序可以使用此操作来最小化 ArrayList 实例的存储量。
     */
    ...
    private void ensureCapacityInternal(int minCapacity) {
        //若为空数组的情况比较 初始化容器大小和期望的最小容量 取最大
     ...
     * 数组分配的最大大小
     * 一些 vm 储备 头关键字 数组中。
     * 试图分配较大的数组可能导致 OutOfMemoryError: 请求数组容量超过 JVM 限制
     */
    private static final int MAX_ARRAY_SIZE = Integer.MAX_VALUE - 8;

    /**
     * 增加容量来确保数据列表能达到指定的最小容量参数
     *
    ...
    private void grow(int minCapacity) {
        ...
        //比较按机制正常扩充容量和所需最小容量大小
        if (newCapacity - minCapacity < 0)
            //若所需最小容量大于正常扩充容量,则以所需最小容量为准
            newCapacity = minCapacity;
        if (newCapacity - MAX_ARRAY_SIZE > 0)
            //如果超出容量最大值,则获取最大容量
            newCapacity = hugeCapacity(minCapacity);
        //最小的数值是通常接近容量,因此这是正确的
     ...
     *
     * 如果指定的数组能容纳队列，并有剩余的空间（即数组的元素比队列多），那么会将数组中紧接 collection 尾部的元素设置为 null。
